...
- Leave `COGNISWEEP_MT_PROVIDER=disabled` for Human Review only, or set `COGNISWEEP_MT_PROVIDER=amazon_translate` to enable the optional managed Amazon Translate route for entitled paid/custom workspaces. User-provided AI API keys take priority.
...
- Managed MT can remain disabled for Human Review only, or Amazon Translate can be enabled for entitled Agency, Enterprise, Unlimited, and owner-approved custom workspaces.
