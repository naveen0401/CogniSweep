...
| P0 | Set up Supabase production DB | External pending | Create production Supabase project, run `supabase_v42_release_schema.sql`, then pass `production_smoke_test.py`. |
...
| P0 | Stop relying on local fallback storage | External pending | Configure Supabase persistence and cloud object storage; smoke/env checks block local fallback for launch. |
| P0 | Configure object storage | External pending | Choose Supabase Storage, S3, or GCS and fill production bucket credentials. |
| P0 | Deploy async worker for heavy QA/Pro jobs | External pending | Deploy `async_task_worker.py` and `async_workflow_processor.py`, set worker URL/token, and run worker smoke tests. |
| P0 | Finalize built-in MT endpoints | External pending | Deploy OPUS-MT and IndicTrans2 endpoints; MADLAD requires production GPU decision. |
| P0 | Test MT route health | External pending | Run `test_builtin_mt_engines.py` after hosted MT endpoints are reachable. |
| P0 | Configure production AI fallback | External pending | Set `OPENAI_API_KEY` or a live managed OpenAI-compatible/vLLM endpoint; env/smoke checks now gate this. |
| P0 | Set production session/auth secrets | External pending | Set production session secret, owner/workspace credentials, and public URL secrets. |
...
| P1 | Run smoke tests for built-in MT | External pending | Start/deploy MT workers and run `test_builtin_mt_engines.py`; verify Pro diagnostics. |
| P1 | IndicTrans2 setup for Indian languages | External pending | Download models, deploy worker, configure `INDICTRANS2_ENDPOINT`, and run health tests. |
| P1 | MADLAD-400 production GPU decision | External pending | Approve or defer GPU-backed MADLAD route, then configure `MADLAD_ENDPOINT` if approved. |
