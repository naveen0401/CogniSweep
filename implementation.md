...
| P0 | Managed MT posture | Repo guard complete | Keep `COGNISWEEP_MT_PROVIDER=disabled` for Human Review only, or enable `amazon_translate` for entitled paid/custom workspaces. |
...
| P1 | Amazon Translate adapter | Implemented as optional route | User-provided AI API keys take priority; Amazon Translate is only used for no-key Agency, Enterprise, Unlimited, or owner-approved custom workspaces. |
...
*   **Completed:** Managed MT posture checker added with retired local engine artifact checks, optional Amazon Translate configuration, release-check integration, and runbook/deployment-pack commands.
...
*   **Completed:** Streamlit Cloud secrets template added with placeholder-only production keys for AI, Supabase, object storage, async workers, billing, email, legal, WAF, backups, and optional managed MT, plus release checks that require the template and block obvious live-secret patterns.
...
*   **Retired:** Local/self-hosted MT batch processing and GPU model-cache work are no longer part of the launch branch; managed no-key MT uses Amazon Translate when enabled for entitled workspaces.
