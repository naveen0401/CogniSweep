...
| P0 | Billing checkout and recurring payments | Repo setup helper complete; external pending | Write live Stripe/Razorpay keys, plan IDs, mandate links, and webhook secret with `python deploy/launch_env_check.py --write-billing-env ...`, then complete provider checkout and reconciliation tests. |
| P0 | Deploy webhook/API receiver | Repo implementation/setup helper complete; external deploy pending | `billing_webhook_receiver.py` is implemented and compose-wired; deploy behind HTTPS and set `ERRORSWEEP_BILLING_WEBHOOK_RECEIVER_URL` with `--write-billing-env`. |
