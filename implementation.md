...
| P0 | Set up Supabase production DB | Repo schema guard complete; external pending | Create production Supabase project, run `python deploy/supabase_schema_check.py --strict`, apply `supabase_v42_release_schema.sql`, then pass `python deploy/supabase_schema_check.py --env-file deploy/.env.production --probe-rest --strict` and `production_smoke_test.py`. |
...
| P0 | Configure object storage | Repo storage guard complete; external pending | Choose Supabase Storage, S3, or GCS, fill production bucket credentials, then pass `deploy/object_storage_check.py` config and probe checks. |
...
| P0 | Finalize built-in MT endpoints | Repo MT guard complete; external endpoint deploy pending | Deploy OPUS-MT and IndicTrans2 endpoints, decide whether MADLAD gets GPU capacity, then pass `python deploy/mt_endpoint_check.py --env-file deploy/.env.production --probe-health --probe-translate --strict`. |
| P0 | Test MT route health | Repo MT guard complete; external route probe pending | Run `python deploy/mt_endpoint_check.py --run-router-smoke --strict` and `test_builtin_mt_engines.py` after hosted MT endpoints are reachable. |
...
| P0 | Live email delivery | External pending | Configure Resend, SendGrid, or SMTP credentials, verify sender domain, deploy dispatch worker, and run deliverability test. |
| P0 | Billing checkout and recurring payments | External pending | Configure live Stripe/Razorpay keys, plan IDs, mandate links, webhook secret, and reconciliation tests. |
| P0 | Deploy webhook/API receiver | Repo implementation complete; external deploy pending | `billing_webhook_receiver.py` is implemented and compose-wired; deploy behind HTTPS and set `ERRORSWEEP_BILLING_WEBHOOK_RECEIVER_URL`. |
...
| P0 | Legal review | External pending | Replace draft legal docs with lawyer-reviewed Terms, Privacy, DPA, Cookie Notice, and customer processing language. |
...
| P1 | Run smoke tests for built-in MT | Repo MT guard complete; external worker smoke pending | Start/deploy MT workers, run `deploy/mt_endpoint_check.py --run-router-smoke`, then verify Pro diagnostics. |
| P1 | IndicTrans2 setup for Indian languages | Repo MT guard complete; external model deploy pending | Download models, deploy worker, configure `INDICTRANS2_ENDPOINT`, and run `deploy/mt_endpoint_check.py --env-file deploy/.env.production --probe-health --strict`. |
| P1 | MADLAD-400 production GPU decision | Repo MT guard complete; external GPU decision pending | Approve or defer GPU-backed MADLAD route, then configure `MADLAD_ENDPOINT` and set `ERRORSWEEP_MADLAD_GPU_APPROVED=true` if approved. |
...
| P1 | End-to-end launch rehearsal | External pending | Run signup to billing/webhook/dashboard rehearsal after all production services and secrets are configured. |
...
*   **Completed:** Platform Settings now includes a manual launch preflight self-test that runs read-only checks for production mode, session secret, public URL, Supabase, object storage, async worker, billing/webhooks, email, legal approval, CDN/WAF, SSO, and self-hosted MT readiness, with a downloadable Markdown report.
*   **Completed:** Platform Settings feature flags are now real owner controls with session persistence, environment-variable overrides, audit logging, and runtime gating for public registration, demo access, billing checkout collection, Pro Human Review, scorecards, subtitle/transcription workflows, main translation routing, and self-hosted no-key MT.
...
*   **Completed:** Supabase and translation-route launch gates hardened with release checks for the production SQL schema, env/template coverage for Supabase anon/service keys, platform AI fallback variables, and live self-hosted MT endpoints for no-key Pro translation.
...
*   **Completed:** Built-in MT endpoint launch checker added with offline router/client/worker contract validation, OPUS/Indic/MADLAD requirements checks, production env checks, optional `/health` and `/translate` probes, router smoke wiring, release-check integration, and runbook/deployment-pack commands.
...
*   **Completed:** Streamlit Cloud secrets template added with placeholder-only production keys for AI, Supabase, object storage, async workers, billing, email, legal, WAF, backups, and self-hosted MT, plus release checks that require the template and block obvious live-secret patterns.
